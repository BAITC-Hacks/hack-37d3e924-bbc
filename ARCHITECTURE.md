...
Контейнер слушает `0.0.0.0:8000`, Docker по умолчанию публикует `127.0.0.1:8000`. SQLite и аудио находятся в volume `/data`; модели смонтированы в `/models` read-only. Образ не содержит ключей, пользовательских данных или весов. Процесс работает непривилегированным пользователем. Части весов хранятся в Git; перед контейнерным запуском `scripts/prepare_repo_models.py` собирает и проверяет их на хосте. Автосборка прямого запуска применяется только к корневому `models/` проекта и не изменяет внешние read-only каталоги.
