...
from .audio import deduplicate_words, owned_words, windows
...
def ctc_words(logits, token_map, offset, duration):
    ...
    def flush():
        ...
        if chars:
            words.append(
                {"text": "".join(chars), "start": offset + start, "end": offset + end}
            )
        chars, start = [], None
...
def ctc_token_map(asr_path):
    tokens = {}
    for line in (
        (Path(asr_path) / "tokens.lst").read_text(encoding="utf-8").splitlines()
    ):
        if line.strip():
    ...
    with sf.SoundFile(str(audio_path)) as audio:
        ...
        if settings.asr == "whisper":
            ...
            model = torch.jit.load(
                str(Path(settings.asr_path) / "model.pt"), map_location=settings.device
            ).eval()
            tokens = ctc_token_map(settings.asr_path)
            for a, b, left, right in windows(duration, core=12, context=0.8):
                audio.seek(int(left * sr))
                clip = audio.read(int((right - left) * sr), dtype="float32")
                with torch.inference_mode():
                    outputs = model(
                        torch.from_numpy(clip).unsqueeze(0).to(settings.device)
                    )
                    logits = (
