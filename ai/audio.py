...
def prepare_audio(source, destination, max_seconds=3600, allowed_formats=None):
    """Decode a local file to mono 16 kHz WAV and return its validated duration."""
    ...
    proc = subprocess.run(
        [
            ...
            "file,pipe",
            *(
                ["-format_whitelist", ",".join(allowed_formats)]
                if allowed_formats
                else []
            ),
            "-i",
    ...
    if not 0.5 <= duration <= max_seconds:
        raise PipelineError(
            "INVALID_AUDIO", "Длительность аудио вне настроенного диапазона."
        )
    return duration
...
def deduplicate_words(words):
    ...
    for word in sorted(words, key=lambda w: (w["start"], w["end"])):
        if result:
            prev = result[-1]
            overlap = max(
                0, min(prev["end"], word["end"]) - max(prev["start"], word["start"])
            )
            shortest = min(prev["end"] - prev["start"], word["end"] - word["start"])
            if (
                word["text"].strip().casefold() == prev["text"].strip().casefold()
                and shortest > 0
                and overlap / shortest > 0.5
            ):
                continue
        result.append(word)
    return result
...
def align(words, turns):
    ...
    for word in deduplicate_words(words):
        candidates = [
            (
                max(0, min(word["end"], t["end"]) - max(word["start"], t["start"])),
                t["speaker_id"],
            )
            for t in turns
        ]
        ...
        uncertain |= (
            ...
            or len([v for v in scores.values() if v > 0]) > 1
        )
        ...
        if (
            ...
            and word["start"] - segments[-1]["end"] < 0.8
            and word["end"] - segments[-1]["start"] < 20
        ):
