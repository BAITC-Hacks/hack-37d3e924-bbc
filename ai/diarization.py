...
def diarize(audio_path, settings):
    ...
    if settings.diarizer == "community1":
        ...
        kwargs = (
            {"num_speakers": settings.num_speakers} if settings.num_speakers else {}
        )
        output = pipeline(
        ...
        turns = [
            {"start": float(t.start), "end": float(t.end), "speaker_id": str(s)}
        ...
        overlap = any(
            a[2] != b[2] and min(a[1], b[1]) > max(a[0], b[0])
            for i, a in enumerate(regular)
            for b in regular[i + 1 :]
            if b[0] < a[1]
        )
    else:
        ...
        cfg = so.OfflineSpeakerDiarizationConfig(
            segmentation=so.OfflineSpeakerSegmentationModelConfig(
                pyannote=so.OfflineSpeakerSegmentationPyannoteModelConfig(
                    model=str(root / "segmentation.onnx")
                ),
                num_threads=settings.threads,
            ),
            embedding=so.SpeakerEmbeddingExtractorConfig(
                model=str(root / "embedding.onnx"), num_threads=settings.threads
            ),
            clustering=so.FastClusteringConfig(
                num_clusters=settings.num_speakers or -1, threshold=0.5
            ),
        ...
        turns = [
            {
                "start": float(t.start),
                "end": float(t.end),
                "speaker_id": f"SPEAKER_{t.speaker:02d}",
            }
