...
def main():
    ...
    for item in json.loads(args.manifest.read_text()):
        with tempfile.TemporaryDirectory(prefix='asr-eval-') as d:
            ...
            started=time.monotonic();words=transcribe(audio,settings);elapsed=time.monotonic()-started
            hypothesis=' '.join(w['text'].strip() for w in words)
            ref,hyp=normal(item['reference']),normal(hypothesis)
            result={'id':item['id'],'duration_seconds':duration,'asr_seconds':round(elapsed,3),
                'rtf':elapsed/duration,'reference_words':len(ref.split()),
                'audio_sha256':hashlib.sha256(Path(item['audio_path']).read_bytes()).hexdigest(),
                'reference_sha256':hashlib.sha256(item['reference'].encode()).hexdigest(),
                'word_edits':distance(ref.split(),hyp.split()),'char_edits':distance(ref,hyp),
