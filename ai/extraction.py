...
def parse_json(text):
    ...
    try:
        ...
        raise PipelineError(
            "INVALID_MODEL_OUTPUT", "Языковая модель вернула некорректный JSON."
        ) from None
...
def resolve_date(phrase, meeting_datetime, timezone):
    ...
    if not phrase:
        return None
    base = (
        datetime.fromisoformat(meeting_datetime.replace("Z", "+00:00"))
    ...
    if any(
        x in text
        for x in (" или ", " либо ", "после согласования", "мүмкін", "белгісіз")
    ):
        return None
    iso = re.fullmatch(
        r"(?:до |к |by )?(\d{4})[.\-/](\d{2})[.\-/](\d{2})", phrase.strip().casefold()
    )
    numeric = re.fullmatch(
        r"(?:до |к )?(\d{1,2})[./](\d{1,2})[./](\d{4})", phrase.strip().casefold()
    )
    try:
        if iso:
        ...
        explicit = re.fullmatch(
            r"(?:до |к )?(\d{1,2}) (" + "|".join(months) + r") (\d{4})(?: года)?", text
        )
        if explicit:
            return date(
                int(explicit[3]), months.index(explicit[2]) + 1, int(explicit[1])
            ).isoformat()
    except ValueError:
        return None
...
class LocalGenerator:
    def __init__(self, settings):
        ...
        if settings.llm == "mlx_torch":
            ...
            self.model, self.tokenizer = load_mlx_torch(
                settings.llm_path, settings.device, settings.threads
            )
            ...
            if settings.quantization == "nf4":
                if settings.device != "cuda":
                    raise PipelineError(
                        "MODEL_UNAVAILABLE", "NF4 настроен только для CUDA."
                    )
            ...
            self.tokenizer = AutoTokenizer.from_pretrained(
                settings.llm_path, local_files_only=True, trust_remote_code=False
            )
            self.model = AutoModelForCausalLM.from_pretrained(
                settings.llm_path, **options
            ).eval()
    ...
    def fits(self, system, payload):
        return (
            len(self.tokenizer.encode(self.prompt(system, payload)))
    ...
    def generate(self, system, payload):
        ...
        try:
            ...
            return parse_json(
                self._generate_text(repair_system, {"invalid_json": text})
            )
    ...
    def _generate_text(self, system, payload):
        ...
        if not self.fits(system, payload):
            raise PipelineError(
                "RESOURCE_EXHAUSTED", "Фрагмент превышает настроенный контекст модели."
            )
        if self.settings.llm == "mlx":
            ...
            token_options = (
                ...
                if self.settings.llm == "mlx_torch"
                else {}
            )
            inputs = self.tokenizer(prompt, return_tensors="pt", **token_options).to(
                self.model.device
            )
...
def segment_batches(segments, fits):
    ...
    for s in segments:
        if not fits([s]):
            raise PipelineError(
                "RESOURCE_EXHAUSTED", "Одна реплика превышает допустимый контекст."
            )
...
def checked_tasks(raw, segments, participants, input_data):
    if not isinstance(raw.get("summary"), str) or not isinstance(
        raw.get("tasks"), list
    ):
        raise PipelineError(
            "INVALID_MODEL_OUTPUT",
            "В ответе языковой модели отсутствуют обязательные поля.",
        )
    ...
    for task in raw["tasks"]:
        if (
            ...
            raise PipelineError(
                "INVALID_MODEL_OUTPUT", "Некорректная структура поручения."
            )
        ...
        if (
            ...
            raise PipelineError(
                "INVALID_MODEL_OUTPUT", "Поручение ссылается на отсутствующую реплику."
            )
        ...
        if (
            owner is not None
            and not isinstance(owner, str)
            or due is not None
            and not isinstance(due, str)
        ):
            raise PipelineError(
                "INVALID_MODEL_OUTPUT", "Некорректный исполнитель или срок."
            )
        assignee = None
        ...
        if (
            ...
            and len(owner) <= 100
            and normal(owner)
            and f" {normal(owner)} " in f" {evidence} "
        ):
            matches = [
                p
                for p in participants
                if p["name"] and normal(p["name"]) == normal(owner)
            ]
            if len(matches) == 1:
        ...
        if due and normal(due) and f" {normal(due)} " in f" {evidence} ":
            due_date = resolve_date(
                due, input_data["meeting_datetime"], input_data["timezone"]
            )
        result.append(
            {
                "id": "pending",
...
def merge_tasks(tasks):
    ...
    for task in tasks:
        match = None
        for old in result:
            same_fields = (old["assignee_id"], old["due_date"]) == (
                task["assignee_id"],
                task["due_date"],
            )
            same_text = normal(old["text"]) == normal(task["text"])
            shared = bool(
                set(old["source_segment_ids"]) & set(task["source_segment_ids"])
            )
            ...
            if same_fields and same_text and shared and old["assignee_id"] is not None:
                ...
                break
        if match:
            match["source_segment_ids"] = list(
                dict.fromkeys(match["source_segment_ids"] + task["source_segment_ids"])
            )
        else:
...
def _explicit_change(quote):
    ...
    return bool(
        re.search(
            r"\b(?:переносим|перенесли|перенесён|перенесен|перенести|"
            r"меняем|изменили|изменён|изменен|ауыстырамыз|ауыстырдық|"
            r"ауыстырылды|өзгертеміз|өзгертілді)\b",
            text,
        )
    )
    ...
    for task in tasks:
        ...
        for old in reversed(result):
            ...
            if old["assignee_id"] is None:
                warnings.append(
                    "Похожие поручения с неизвестным исполнителем оставлены отдельно; проверьте повторы и сроки."
                )
                continue
            ids = sorted(
                set(old["source_segment_ids"] + task["source_segment_ids"]),
                key=order.get,
            )
            payload = {
                "earlier_task": old,
            ...
            if comparisons >= comparison_limit or not generator.fits(
                RECONCILE_SYSTEM, payload
            ):
                warnings.append(
                    "Не все похожие поручения удалось сопоставить в лимите контекста/сравнений; проверьте повторы и сроки."
                )
                continue
            comparisons += 1
            ...
            if decision not in ("keep", "duplicate", "replace_deadline"):
                raise PipelineError(
                    "INVALID_MODEL_OUTPUT",
                    "Некорректный результат сопоставления поручений.",
                )
            if decision == "duplicate":
                ...
                accepted = old["due_date"] == task["due_date"] and (
                    old["due_date"] is not None or shared
                )
            elif decision == "replace_deadline":
                sid, quote, due = (
                    raw.get("source_segment_id"),
                    raw.get("quote"),
                    raw.get("due_text"),
                )
                accepted = (
                    isinstance(sid, str)
                    and sid in task["source_segment_ids"]
                    and order[sid] > max(order[i] for i in old["source_segment_ids"])
                    and isinstance(quote, str)
                    and bool(quote.strip())
                    and quote in by_id[sid]["text"]
                    and _explicit_change(quote)
                    and _explicit_change(by_id[sid]["text"])
            ...
            if accepted:
                ...
                break
            warnings.append(
                "Похожие поручения оставлены отдельно: повтор или изменение срока не подтверждены однозначно. Проверьте источники."
            )
        if not merged:
            result.append(deepcopy(task))
...
def extract(segments, input_data, settings):
    if not segments:
        return {
            "tasks": [],
            "summary": "Речь не обнаружена.",
            "warnings": ["Проверьте, что запись действительно не содержит речи."],
        }
    ...
    while len(summaries) > 1:
        ...
        if len(groups) >= len(summaries):
            raise PipelineError(
                "RESOURCE_EXHAUSTED",
                "Саммари частей не помещаются в контекст объединения.",
            )
        summaries = []
        for group in groups:
            ...
            if not isinstance(raw.get("summary"), str):
                raise PipelineError(
                    "INVALID_MODEL_OUTPUT", "Некорректное итоговое саммари."
                )
            summaries.append(raw["summary"])
    ...
    return {
        ...
        "summary": summaries[0],
        "warnings": [
            "Поручения и саммари — черновик: проверьте смысл, исполнителей, сроки и возможные повторы.",
            "Имена исполнителей сопоставляются только с заранее переданными участниками; неизвестные остаются null.",
        ]
        + reconciliation_warnings
        + (
            [
                "Удалена лишняя завершающая кавычка в ответе локальной модели; результат прошёл повторную проверку JSON."
            ]
            if gen.format_repairs
            else []
        ),
    }
