...
def _unsupported(detail):
    raise PipelineError(
        "MODEL_UNAVAILABLE", "Unsupported original MLX snapshot: " + detail
    )
...
class PackedAffine4(nn.Module):
    ...
    def dequantize_rows(self, selection):
        ...
        codes = ((weight.unsqueeze(-1) >> shifts) & 15).reshape(
            weight.shape[0], -1, GROUP_SIZE
        )
...
class PackedEmbedding(nn.Module):
    ...
    def forward(self, input_ids):
        ...
        output = torch.empty(
            ...
            device=self.packed.weight.device,
        )
...
def _validate_config(config):
    ...
    if (
        ...
        or config.get("rope_parameters") is not None
        or config.get("use_sliding_window", False)
    ):
        _unsupported(
            "modified rotary or sliding-window configuration is not supported."
        )
    for key in ("quantization", "quantization_config"):
        ...
        if (
            ...
            or quant.get("bits") != 4
            or quant.get("group_size") != GROUP_SIZE
        ):
...
def _load_packed_weights(model, tensors, device):
    ...
    for name, module in modules:
        packed = PackedAffine4(
            *(
                tensors.get_tensor(name + suffix).to(device)
                for suffix in (".weight", ".scales", ".biases")
            )
        )
        if (packed.rows, packed.columns) != tuple(module.weight.shape):
            _unsupported(
                "packed matrix dimensions do not match the model configuration."
            )
        replacement = (
            PackedEmbedding(packed)
            if isinstance(module, nn.Embedding)
            else PackedLinear(packed)
        )
    ...
    for name, parameter in ordinary:
        ...
        if tensor.dtype != torch.bfloat16 or tuple(tensor.shape) != tuple(
            parameter.shape
        ):
            _unsupported(
                "normalization tensor dtype or shape does not match the model."
            )
        parent, _, leaf = name.rpartition(".")
...
def load_mlx_torch(model_path, device="cpu", threads=4):
    ...
    if not all(
        ...
        _unsupported(
            "the original local weights or generation configuration are missing."
        )
