...
def default_state_dir():
    if hasattr(os, "getuid"):
        ...
        scope = (
            "".join(
                c if c.isalnum() or c in ("-", "_") else "_" for c in getpass.getuser()
            )
            or "user"
        )
...
def run_pipeline(input_data, on_progress=None):
    """Validate one meeting, run local stages under a lock, and return its result."""
    ...
    if mode == "fixture":
        ...
        if input_data != example:
            raise PipelineError(
                "INVALID_INPUT",
                "Тестовый режим принимает только общий синтетический пример.",
            )
        if on_progress:
            on_progress({"stage": "validating"})
        return validate_result(
            json.loads((fixture / "result.json").read_text(encoding="utf-8")),
            input_data,
        )
    if mode != "real":
    ...
    try:
        ...
        raise PipelineError(
            "RESOURCE_EXHAUSTED", "ИИ-модуль уже обрабатывает другую запись."
        ) from None
    with lock:
        with tempfile.TemporaryDirectory(prefix="meeting-", dir=state_dir) as temporary:
            ...
            (folder / "request.json").write_text(
                json.dumps(
                    {"input": input_data, "settings": settings.to_dict()},
                    ensure_ascii=False,
                ),
                encoding="utf-8",
            )
            ...
            def stage(name):
                ...
                try:
                    proc = subprocess.run(
                        [sys.executable, "-m", "ai.worker", name, str(folder)],
                        env=env,
                        cwd=root,
                    ...
                    raise PipelineError(
                        "PROCESSING_FAILED", "Превышено время обработки этапа."
                    ) from None
                if proc.returncode:
                    ...
                    raise PipelineError(
                        "PROCESSING_FAILED",
                        "Процесс ИИ завершился до получения результата.",
                    )
                metrics.append(
                    json.loads(
                        (folder / f"{name}.metrics.json").read_text(encoding="utf-8")
                    )
                )
            ...
            if not words and diarization["turns"]:
                raise PipelineError(
                    ...
                    "Речь обнаружена, но распознавание не дало текста.",
                )
            if words and not diarization["turns"]:
                raise PipelineError(
                    "INVALID_MODEL_OUTPUT",
                    "Диаризация не определила речь в распознанной записи.",
                )
            ...
            warnings = analysis["warnings"] + [
                "Автоматическая расшифровка: проверьте имена, числа и исходный язык.",
                "Метки говорящих сами по себе не подтверждают личность.",
            ]
            if uncertain or diarization["overlap"]:
                warnings.append(
                    "Есть пересечение голосов или реплики с неопределённым говорящим; проверьте таймкоды."
                )
            result = {
                "schema_version": 1,
                **{
            ...
            if os.environ.get("AI_METRICS_PATH"):
                ...
                Path(os.environ["AI_METRICS_PATH"]).write_text(
