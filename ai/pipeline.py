...
def run_pipeline(input_data, on_progress=None):
    ...
    import fcntl
    state_dir=Path(os.environ.get('AI_STATE_DIR',str(Path(tempfile.gettempdir())/f'meeting-ai-{os.getuid()}')))
    state_dir.mkdir(parents=True,exist_ok=True,mode=0o700)
    with (state_dir/'inference.lock').open('a') as lock:
        try:
        ...
        with tempfile.TemporaryDirectory(prefix='meeting-',dir=state_dir) as temporary:
            ...
            (folder/'request.json').write_text(json.dumps({'input':input_data,'settings':settings.to_dict()},ensure_ascii=False))
            metrics=[]
            ...
            def stage(name):
                ...
                if proc.returncode:
                    ...
                    if error.exists():
                        detail=json.loads(error.read_text())
                ...
                metrics.append(json.loads((folder/f'{name}.metrics.json').read_text()))
            ...
            (folder/'segments.json').write_text(json.dumps(segments,ensure_ascii=False))
            ...
            if os.environ.get('AI_METRICS_PATH'):
                ...
                Path(os.environ['AI_METRICS_PATH']).write_text(json.dumps(report,indent=2))
            return result
