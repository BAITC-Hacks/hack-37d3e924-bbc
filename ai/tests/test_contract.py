...
def pair():
    return tuple(
        json.loads((ROOT / "fixtures" / f"{name}.json").read_text(encoding="utf-8"))
...
def test_fixture_explicit_and_valid(monkeypatch, pair):
    ...
    altered["meeting_id"] = "real-upload"
    with pytest.raises(PipelineError):
        run_pipeline(altered)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        lambda r: (
            r["participants"][0].update(speaker_ids=["same"])
            or r["participants"][1].update(speaker_ids=["same"])
        ),
...
def test_invalid_time(pair, dt, zone):
    request, _ = pair
    request.update(meeting_datetime=dt, timezone=zone)
    with pytest.raises(PipelineError):
