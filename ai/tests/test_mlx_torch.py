...
# Torch-dependent imports must follow the optional dependency skip.
# ruff: noqa: E402
...
from ai.mlx_torch import (
    MAX_ROW_CHUNK,
    PackedAffine4,
    PackedEmbedding,
    PackedLinear,
    _load_packed_weights,
    _validate_config,
    load_mlx_torch,
)
...
def pack(codes):
    ...
    return (
        (codes.reshape(codes.shape[0], -1, 8).long() << shifts).sum(-1).to(torch.int32)
    )


def matrix(rows=5, columns=64, row_chunk=2):
    codes = torch.arange(rows * columns).reshape(rows, columns) % 16
    scales = torch.full((rows, columns // 64), 0.125, dtype=torch.bfloat16)
    biases = torch.full_like(scales, -0.75)
...
def test_packed_sign_bit_low_nibble_order_and_affine_groups():
    ...
    scales = torch.tensor([[0.125, -0.25]], dtype=torch.bfloat16)
    biases = torch.tensor([[-0.75, 2.5]], dtype=torch.bfloat16)
    packed = PackedAffine4(weight.view(torch.uint32), scales, biases)
    expected = (
        (
            codes.reshape(1, 2, 64).float() * scales.float().unsqueeze(-1)
    ...
    torch.testing.assert_close(
        packed.dequantize_rows(slice(0, 1)), expected, rtol=0, atol=0
    )
...
def test_affine_expression_rounds_to_bfloat16_only_once():
    ...
    bias = torch.tensor([[-0.03125]], dtype=torch.bfloat16)
...
def test_embedding_and_vocab_head_share_buffers_and_match_dense_math():
    ...
    for name in ("weight", "scales", "biases"):
        assert (
            getattr(embedding.packed, name).data_ptr()
            == getattr(head.packed, name).data_ptr()
        )
    dense = (codes.float() * 0.125 - 0.75).to(torch.bfloat16)
    indices = torch.tensor([[4, 1, 4], [0, 2, 3]])
    torch.testing.assert_close(
        embedding(indices), F.embedding(indices, dense), rtol=0, atol=0
    )
...
def test_local_loader_and_bounded_greedy_generation_without_network(
    ...
    monkeypatch.setattr(
        transformers.Qwen3Config, "from_dict", classmethod(lambda cls, data: config)
    )
    tokenizer_calls = []
    tokenizer = object()
    ...
    def forbidden(*args, **kwargs):
        raise AssertionError(
            "synthetic model loading/generation attempted network access"
        )
    ...
    hook = model.lm_head.register_forward_pre_hook(
        lambda module, args: sizes.append(args[0].shape[-2])
    )
    try:
...
def test_settings_select_explicit_original_runtime_with_small_defaults(
    tmp_path, monkeypatch
):
    ...
    for key, value in {
        ...
        "AI_DIARIZER": "sherpa",
        "AI_DEVICE": "cpu",
        "AI_ASR_PATH": str(tmp_path),
        "AI_DIARIZATION_PATH": str(tmp_path),
        "AI_LLM_PATH": str(tmp_path),
    }.items():
        monkeypatch.setenv(key, value)
    (tmp_path / "model.pt").touch()
    (tmp_path / "tokens.lst").touch()
    settings = Settings.from_env()
    assert (settings.llm, settings.context_tokens, settings.max_new_tokens) == (
        "mlx_torch",
        2048,
        512,
    )
