...
from ai import model_check as check
...
class ModelCheckTests(unittest.TestCase):
    ...
    def test_cpu_success_does_not_hash_or_load_weights(self):
        ...
        result = check.check_models(self.models, 'cpu')
    ...
    def test_hash_verification_streams_synthetic_files(self):
        result = check.check_models(self.models, 'cpu', verify_hashes=True)
        self.assertTrue(result['ok'], result)
    ...
    def test_cuda_missing_never_falls_back(self):
        ...
        result = check.check_models(self.models, 'cuda')
        self.assertFalse(result['ok'])
