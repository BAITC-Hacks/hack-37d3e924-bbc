...
from ai.audio import align, deduplicate_words, owned_words, windows
...
@pytest.mark.parametrize(
    ...
    [
        ("завтра", "2026-09-24"),
        ("к завтрашнему дню", "2026-09-24"),
        ("ертеңге дейін", "2026-09-24"),
        ("через неделю", "2026-09-30"),
        ("бір апта ішінде", "2026-09-30"),
        ("екі аптадан кейін", "2026-10-07"),
        ("үш күн ішінде", "2026-09-26"),
        ("через 3 дня", "2026-09-26"),
        ("3 күн ішінде", "2026-09-26"),
        ("до 25 сентября 2026", "2026-09-25"),
        ("2026-09-25", "2026-09-25"),
        ("до 25.09.2026", "2026-09-25"),
        ("пятница", None),
        ("до конца недели", None),
        ("после согласования", None),
        ("30.02.2026", None),
        ("завтра или послезавтра", None),
        ("за три рабочих дня", None),
    ],
)
...
def test_long_boundary_times_and_repetitions():
    ...
    source = [
        {"start": 23.7, "end": 24.3, "text": "отчёт"},
        {"start": 24.4, "end": 24.7, "text": "да"},
        {"start": 24.8, "end": 25.1, "text": "да"},
        {"start": 48, "end": 48.5, "text": "дайын"},
    ]
    out = []
    for a, b, left, right in windows(55):
        out += owned_words(
            [w for w in source if w["start"] >= left and w["end"] <= right], a, b
        )
    assert deduplicate_words(out) == source
    turns = [
        {"start": 0, "end": 30, "speaker_id": "A"},
        {"start": 30, "end": 55, "speaker_id": "B"},
    ]
...
def test_owner_is_not_speaker_and_quotes_required():
    ...
    people = [
        {"id": "boss", "name": "Марсель", "speaker_ids": ["A"]},
        {"id": "p2", "name": "Айжан", "speaker_ids": []},
    ]
    segments = [
        {
            "id": "s1",
            "speaker_id": "A",
            "text": "Айжан, подготовь отчёт к завтрашнему дню.",
        }
    ]
    raw = {
        "summary": "Отчёт",
    ...
    assert (
        ...
        and task["needs_review"]
    )
    raw["tasks"][0]["source_segment_ids"] = ["injected"]
    with pytest.raises(PipelineError):
        checked_tasks(raw, segments, people, req)
