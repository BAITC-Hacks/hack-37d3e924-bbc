...
class WindowsCheckTests(unittest.TestCase):
    def setUp(self):
        ...
        files = {
            "llm/config.json": b'{"synthetic_fixture":true}',
        ...
        manifest.write_text(
            json.dumps({"llm_revision": "synthetic-revision", "files": entries}),
            encoding="utf-8",
        )
        ...
        self.cuda = SimpleNamespace(
            ...
            get_device_properties=Mock(
                return_value=SimpleNamespace(name="Synthetic GPU")
            ),
            mem_get_info=Mock(return_value=(3 * 1024**3, 4 * 1024**3)),
        )
        self.torch = SimpleNamespace(
            version=SimpleNamespace(cuda="synthetic-cuda"), cuda=self.cuda
        )
        self.imports = self.patch(
            check.importlib,
            "import_module",
            Mock(
                side_effect=lambda name: (
                    self.torch if name == "torch" else SimpleNamespace()
                )
            ),
        )
        self.patch(
            check.importlib.metadata, "version", Mock(return_value="synthetic-version")
        )
    ...
    def test_cpu_success_does_not_hash_or_load_weights(self):
        hasher = self.patch(
            check, "_sha256", Mock(side_effect=AssertionError("Unexpected weight read"))
        )
    ...
    def test_invalid_config_and_cuda_driver_errors_are_sanitized(self):
        ...
        self.assertEqual(
            ...
            ["MODEL_FORMAT_INVALID", "CUDA_CHECK_FAILED"],
        )
        self.assertNotIn("SECRET", json.dumps(result))
