...
def main():
    ...
    try:
        if stage == "preparing_audio":
            ...
            segments = json.loads(
                (folder / "segments.json").read_text(encoding="utf-8")
            )
        ...
        metrics = {
            ...
            "torch_peak_cuda_allocated_bytes": cuda_peak,
            "mlx_peak_allocated_bytes": mlx_peak,
        ...
        (folder / f"{stage}.metrics.json").write_text(
        ...
        if isinstance(e, PipelineError):
            ...
            code, message = (
                "RESOURCE_EXHAUSTED",
                "Недостаточно памяти для локальной модели.",
            )
        elif isinstance(e, (ImportError, FileNotFoundError)):
            code, message = (
                "MODEL_UNAVAILABLE",
                "Не найдены локальные модели или зависимости.",
            )
        else:
            code, message = (
                "PROCESSING_FAILED",
                "Локальная модель не завершила обработку.",
            )
