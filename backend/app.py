...
def create_app(settings=None):
    """Build the HTTP app and storage boundary without loading inference models."""
    ...
    async def validation_error(request, error):
        return await api_error(
            request, APIError(422, "VALIDATION_ERROR", "Проверьте данные")
        )
    ...
    async def http_error(request, error):
        ...
        return await api_error(
            request,
            APIError(status, codes.get(status, "VALIDATION_ERROR"), "Проверьте запрос"),
        )

    @app.exception_handler(Exception)
    async def unexpected_error(request, error):
        return await api_error(
            request, APIError(500, "INTERNAL_ERROR", "Не удалось выполнить операцию")
        )
    ...
    async def create_meeting(audio: UploadFile = File(...), metadata: str = Form(...)):
        ...
        if suffix not in {".wav", ".mp3", ".m4a", ".flac"}:
            raise APIError(
                415, "UNSUPPORTED_AUDIO", "Используйте WAV, MP3, M4A или FLAC"
            )
        ...
        try:
            ...
            data = {
                ...
                **{
                    key: meta[key]
                    for key in ("meeting_datetime", "timezone", "participants")
                },
            }
            validate_input(data)
        except Exception:
            raise APIError(
                422,
                "VALIDATION_ERROR",
                "Проверьте название, дату, часовой пояс и участников",
            ) from None
        created = False
        try:
            count = 0
            ...
            try:
                ...
                raise APIError(
                    415,
                    "UNSUPPORTED_AUDIO",
                    "Аудио не декодируется или имеет недопустимую длительность",
                ) from None
    ...
    def export(meeting_id: str):
        ...
        return Response(
            ...
            headers={
                "Content-Disposition": f'attachment; filename="meeting-{meeting_id}.docx"'
            },
        )
