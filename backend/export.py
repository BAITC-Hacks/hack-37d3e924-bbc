...
def export_docx(meeting):
    """Render the supplied saved meeting revision as an in-memory DOCX."""
    ...
    if meeting["mode"] == "fixture":
        document.add_paragraph(
            "ТЕСТОВЫЙ РЕЗУЛЬТАТ — синтетический пример; модели не запускались."
        )
    document.add_heading(meeting["title"], 0)
    document.add_paragraph(
        f"{meeting['meeting_datetime']} • {meeting['timezone']} • Версия {meeting['revision']}"
    )
    document.add_heading("Участники", 1)
    names = {p["id"]: p["name"] or "требует уточнения" for p in result["participants"]}
    for participant in result["participants"]:
        document.add_paragraph(
            f"{names[participant['id']]} ({participant['id']}); голоса: "
    ...
    for task in result["tasks"]:
        ...
        document.add_paragraph(
            f"Исполнитель: {names.get(task['assignee_id'], 'требует уточнения')}. "
