...
from backend.store import APIError, Store
...
def upload(client, **overrides):
    metadata = {'title': ' Обсуждение ', 'meeting_datetime': '2026-09-23T10:00:00+05:00',
                'timezone': 'Asia/Almaty', 'participants': [{'id': 'person-a', 'name': 'Дарын', 'speaker_ids': []}]}
    metadata.update(overrides)
    return client.post('/api/v1/meetings', data={'metadata': json.dumps(metadata)},
...
def review_body(meeting):
    return {'expected_revision': meeting['revision'],
            **{k: deepcopy(meeting['result'][k]) for k in ('participants', 'tasks', 'summary')}}
...
@pytest.mark.parametrize('mutation', ['source', 'date', 'assignee', 'speaker', 'review', 'participants', 'empty'])
...
def test_claim_delete_recovery_retry_and_state_guards(setup):
    ...
    assert failed['error'] == {'code': 'PROCESSING_FAILED', 'message': 'Обработка не завершена', 'details': {}}
...
def test_real_errors_never_become_fixture_and_retry_same_id(tmp_path, monkeypatch):
    ...
    assert client.post('/api/v1/meetings/' + meeting['id'] + '/retry').json()['id'] == meeting['id']
...
def test_invalid_audio_json_metadata_and_limits(setup, tmp_path):
    ...
    response = client.post('/api/v1/meetings', data={'metadata': '{}'}, files={'audio': ('bad.exe', b'x')})
    assert response.status_code == 415
    meta = {'title': 'Test', 'meeting_datetime': '2026-09-23T10:00:00+05:00', 'timezone': 'Asia/Almaty', 'participants': []}
    for metadata, content, expected in [('{', audio_bytes(), 422), (json.dumps(meta), b'broken file', 415)]:
        response = client.post('/api/v1/meetings', data={'metadata': metadata}, files={'audio': ('bad.wav', content)})
        assert response.status_code == expected
        assert 'error' in response.json()
    assert list(settings.audio_dir.iterdir()) == []
    small = TestClient(create_app(Settings(tmp_path / 'small', tmp_path / 'small/db', 'fixture', 100)))
...
def test_renamed_local_playlist_is_rejected(setup, tmp_path):
    ...
    meta = {'title': 'Test', 'meeting_datetime': '2026-09-23T10:00:00+05:00', 'timezone': 'Asia/Almaty', 'participants': []}
    response = client.post('/api/v1/meetings', data={'metadata': json.dumps(meta)},
...
def test_actual_worker_process_fixture_and_singleton_lock(setup):
    ...
    with lockfile.open('a') as lock:
        ...
        assert store.get(meeting['id'])['status'] == 'queued'
    completed = subprocess.run(command, env=environment, capture_output=True, timeout=15)
...
def test_direct_worker_shutdown_stops_stage_without_signalling_caller(tmp_path):
    ...
    environment = {**os.environ, 'DATA_DIR': str(tmp_path), 'DATABASE_PATH': str(settings.database),
                   'PIPELINE_MODE': 'real', 'TEST_STAGE_PID': str(child_ready)}
    ...
    try:
        ...
        assert store.get(meeting['id'])['status'] == 'failed'
        assert client.post('/api/v1/meetings/' + meeting['id'] + '/retry').status_code == 202
    finally:
