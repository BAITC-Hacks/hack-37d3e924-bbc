...
Реализованный контракт React ↔ FastAPI. Формат результата задаёт [result.schema.json](result.schema.json), формат входа ИИ — [input.schema.json](input.schema.json). HTTP-поля находятся в оболочке, а не в результате ИИ. Изменений JSON Schema этот документ не вводит.
...
title: 1–200 символов после trim; остальные поля по входной схеме. Сервер генерирует meeting_id и безопасный audio_path. По умолчанию: 100 МиБ, WAV/MP3/M4A/FLAC; сервер проверяет размер потока и фактическое декодирование. Успех только после сохранения файла и задания. Ответ 202 Meeting.
...
- stage: null либо preparing_audio | transcribing | diarizing | aligning | extracting_tasks | summarizing | validating; при done/failed null.
...
Общий fixture → UI → правка → PUT → повторный GET → DOCX с этой правкой. Затем тот же сценарий mode=real с worker и локальными весами. Битый JSON/сбой → failed. После рестарта правки сохраняются. Сценарий приёмки и границы измеренного качества: [README](../README.md).
