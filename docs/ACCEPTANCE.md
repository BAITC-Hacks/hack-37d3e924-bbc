...
Статус: все продуктовые испытания сейчас `NOT RUN`, потому что нет подтвержденного приложения, аудио, локальных моделей и прогонов. Проверка `contracts/verify.ps1` может быть `PASS`, но она доказывает только валидность схем и фикстур, а не работу продукта.
