...
def validate_manifest(manifest):
    ...
    for item in manifest["files"]:
        ...
        for asset in item["assets"]:
            name = asset["name"]
            if not re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9._-]*", name) or name in assets:
...
def fetch_asset(manifest, asset, cache):
    ...
    if result.returncode:
        raise RuntimeError(
            "Download failed. Check gh auth status and repository access; "
...
def install(manifest, destination, components=None, verify_only=False):
    ...
    targets = {
        item["path"]: safe_target(destination, item["path"])
        for item in manifest["files"]
    }
