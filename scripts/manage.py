...
def default_python():
    return str(
        ROOT / (".venv/Scripts/python.exe" if IS_WINDOWS else ".venv/bin/python")
    )
...
def environment(args):
    env = dict(os.environ)
    env.update(
        PIPELINE_MODE="fixture" if args.profile == "fixture" else "real",
    ...
    if args.profile == "mac":
        if platform.system() != "Darwin" or platform.machine() != "arm64":
            raise SystemExit(
                "Профиль mac требует macOS Apple Silicon. Для Linux используйте --profile cuda."
            )
        models = args.models or env.get("MEETING_MODEL_DIR") or ROOT / "models"
        folder = Path(models).expanduser().resolve()
        env.update(
            AI_ASR="mixed_ctc",
            AI_ASR_PATH=str(folder / "asr"),
            AI_DIARIZER="sherpa",
            AI_DIARIZATION_PATH=str(folder / "diarization"),
            AI_LLM="mlx",
            AI_LLM_PATH=str(folder / "llm"),
            AI_DEVICE="cpu",
        )
    elif args.profile == "windows":
        folder = (
            Path(args.models or env.get("MEETING_MODEL_DIR") or ROOT / "models")
            .expanduser()
            .resolve()
        )
        env.update(
            MEETING_MODEL_DIR=str(folder),
            AI_ASR="mixed_ctc",
            AI_ASR_PATH=str(folder / "asr"),
            AI_DIARIZER="sherpa",
            AI_DIARIZATION_PATH=str(folder / "diarization"),
...
def run(args):
    ...
    if args.profile != "fixture":
        ...
        call(
            [
                args.worker_python or args.python,
                "-c",
                "from ai.settings import Settings; Settings.from_env()",
            ],
            env,
        )
    children = []
    ...
    try:
        for cmd in (
            [args.worker_python or args.python, "-m", "backend.worker"],
            [
                args.python,
                "-m",
                "uvicorn",
                ...
                "127.0.0.1",
                "--port",
                env.get("BACKEND_PORT", "8000"),
            ],
        ):
            children.append(
                subprocess.Popen(cmd, cwd=ROOT, env=env, **worker_popen_kwargs())
            )
        print(
            "Приложение: http://127.0.0.1:" + env.get("BACKEND_PORT", "8000"),
            flush=True,
        )
        print(
            "Режим: "
            + (
                "ТЕСТОВАЯ ФИКСТУРА — модели не запускаются"
                if args.profile == "fixture"
        ...
        while all(child.poll() is None for child in children):
            time.sleep(0.3)
        raise SystemExit(
            "Один из процессов завершился. Остановлены и API, и worker; проверьте сообщение выше."
        )
...
def main():
    ...
    parser.add_argument(
        "--profile",
        choices=["mac", "cuda", "windows", "fixture"],
        default="windows"
        if IS_WINDOWS
        else "mac"
    ...
    parser.add_argument(
        "--worker-python", help="Optional separate preinstalled AI environment"
    )
    ...
    if args.action == "setup":
        ...
        if args.profile == "windows" and args.device == "cuda":
            pip_command += [
                "torch==2.14.0+cu130",
                "--extra-index-url",
        ...
        if args.profile in ("mac", "windows"):
            call([args.python, ROOT / "scripts/prepare_repo_models.py"])
