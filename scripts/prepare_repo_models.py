...
def validate(manifest, directory):
    ...
    for item in manifest["files"]:
        ...
        if "parts" in item and (
            not item["parts"]
            or sum(part["bytes"] for part in item["parts"]) != item["bytes"]
        ):
    ...
    for item in manifest["files"]:
        ...
        for part in item["parts"]:
            if not verified(safe_target(directory, part["path"]), part):
                raise RuntimeError(
                    "Часть модели отсутствует или повреждена: "
                    + part["path"]
                    + ". Получите полный репозиторий через git pull."
                )
        ...
        try:
            ...
            if not verified(temporary, item):
                raise RuntimeError(
                    "SHA-256 собранной модели не совпал: " + item["path"]
                )
...
def main(argv=None):
    ...
    parser.add_argument(
        "--component", action="append", choices=["asr", "diarization", "llm"]
    )
    parser.add_argument("--verify-only", action="store_true")
    args = parser.parse_args(argv)
    manifest = json.loads(
        (ROOT / "models/repository-models.lock.json").read_text(encoding="utf-8")
    )
