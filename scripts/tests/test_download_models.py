...
spec = importlib.util.spec_from_file_location(
    "release_download", Path(__file__).parents[1] / "download_models.py"
)
...
def test_split_download_assembly_and_offline_reuse(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        download.subprocess, "run", lambda _: pytest.fail("Unexpected network")
    )
...
@pytest.mark.parametrize(
    "name", ["../escape", "/absolute", "asr/../../escape", "C:\\escape"]
)
...
def test_symlink_escape_rejected(tmp_path):
    ...
    destination.mkdir()
    outside.mkdir()
    link = destination / "asr"
    try:
        link.symlink_to(outside, target_is_directory=True)
    except OSError as error:
        if getattr(error, "winerror", None) != 1314:
            raise
        # A directory junction exercises the same escape without requiring
        # Windows Developer Mode or the symlink privilege.
        import _winapi

        _winapi.CreateJunction(str(outside), str(link))
    with pytest.raises(ValueError, match="escapes"):
        download.install(manifest(), destination)
...
def test_release_preserves_original_model_pins():
    ...
    for source in ["ai/mac-models.lock.json"]:
