...
from scripts import manage, prepare_repo_models as bundle
...
def fixture(tmp_path):
    parts = [record('asr/model.pt.part-001', b'first'),
             record('asr/model.pt.part-002', b'second')]
    for part, data in zip(parts, (b'first', b'second')):
        path = tmp_path / part['path']
        path.parent.mkdir(exist_ok=True)
        path.write_bytes(data)
    return {'schema_version': 1, 'files': [{**record('asr/model.pt', b'firstsecond'), 'parts': parts}]}
...
def test_assemble_then_verify_and_reuse(tmp_path):
    ...
    assert all((tmp_path / part['path']).exists() for part in manifest['files'][0]['parts'])
...
def test_bundle_matches_both_original_manifests():
    ...
    for name in ['ai/mac-models.lock.json', 'models/source-models.lock.json']:
        original = json.loads((root / name).read_text())
        for item in original['files']:
            assert all(by_path[item['path']][key] == value for key, value in item.items())
    assert all(part['bytes'] <= 48 * 1024**2 for item in manifest['files']
               for part in item.get('parts', [item]))


def test_launcher_defaults_to_bundled_models_and_preserves_overrides(monkeypatch, tmp_path):
...
@pytest.mark.parametrize('profile', ['mac', 'windows', 'brev'])
def test_only_bundled_paths_are_assembled_before_launch(profile, monkeypatch, tmp_path):
    ...
    manage.prepare_bundled_models(args, {'AI_ASR_PATH': str(manage.ROOT / 'models/asr')})
    assert calls == [[manage.sys.executable, manage.ROOT / 'scripts/prepare_repo_models.py']]
    calls.clear()
