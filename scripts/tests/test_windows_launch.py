...
spec = importlib.util.spec_from_file_location(
    "manage_windows", Path(__file__).parents[1] / "manage.py"
)
manage = importlib.util.module_from_spec(spec)
spec.loader.exec_module(manage)


def test_root_python_is_selected_even_when_environment_is_not_installed(
    tmp_path, monkeypatch
):
    monkeypatch.setattr(manage, "ROOT", tmp_path)
    monkeypatch.setattr(manage, "IS_WINDOWS", True)
    assert manage.default_python() == str(tmp_path / ".venv/Scripts/python.exe")


def test_windows_default_models_are_bundled_in_repository(monkeypatch):
    monkeypatch.delenv("MEETING_MODEL_DIR", raising=False)
    args = SimpleNamespace(profile="windows", models=None, device="cpu")
    assert manage.environment(args)["MEETING_MODEL_DIR"] == str(manage.ROOT / "models")
...
def test_doctor_uses_ai_environment_and_verifies_hashes(tmp_path, monkeypatch):
    ...
    calls = []
    monkeypatch.setattr(
        manage, "call", lambda cmd, environment: calls.append((cmd, environment))
    )
